...
Finally, the “Details” column tells you the values you can expect to see for each variable in the dataset. The first piece of information under "Details" tells you the data type of the variable: 

- The "character" type is a sequence of one or more text characters, like a word or phrase
- The "numeric" type represents a decimal number.

In brackets, you'll see the values the variable can take on.  Some variables can only take on a few values. For example, 

- `Survived` can either equal 0 or 1.
- `Sex` is either `male` or `female`.
- `Pclass` is equal to 1, 2 or 3.

For numeric variables that are measurements, the brackets will identify the units of measure. For example, Age is measured in years and Fare is measured in £. 
